  ...
  ideas: loadItems(storageKeys.ideas, [
    {
      ...
      title: "Domingo de desayuno y vuelta tranquila",
      note: "Sin prisa, solo un lugar rico y tiempo juntos.",
    },
  ]),
  plans: loadItems(storageKeys.plans, [
    {
      id: crypto.randomUUID(),
      title: "Tarde de cafe y libreria",
      date: getUpcomingDate(4),
      time: "18:30",
      status: "En progreso",
      note: "Ver si tambien quieren caminar un rato despues.",
    },
  ]),
};
...
function renderIdeas() {
  ...
  if (!state.ideas.length) {
    ideasList.appendChild(createEmptyState("Aqui pueden ir soltando ideas rapidas para no dejarlas escapar."));
    return;
  }
  ...
  state.ideas.forEach((idea) => {
    ...
    card.innerHTML = `
      ...
      <div class="entry-actions">
        <button class="ghost-button" type="button" data-action="promote-idea" data-id="${idea.id}">Pasar a plan</button>
...
function renderPlans() {
  ...
  if (!state.plans.length) {
    plansList.appendChild(createEmptyState("Cuando definan fecha y hora, sus planes apareceran aqui."));
    return;
  }
    ...
    card.innerHTML = `
      ...
      <div class="entry-actions">
        <button class="ghost-button" type="button" data-action="toggle-plan" data-id="${plan.id}">
          ${isConfirmed ? "Marcar en progreso" : "Confirmar plan"}
        </button>
